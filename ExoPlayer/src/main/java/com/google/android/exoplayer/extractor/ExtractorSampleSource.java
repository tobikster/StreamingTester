 ...
 */
public
class ExtractorSampleSource implements SampleSource, ExtractorOutput, Loader.Callback {
	 ...
	 */
	public
	ExtractorSampleSource(Uri uri, DataSource dataSource, Extractor extractor,
	...
	@Override
	public
	void onLoadError(Loadable ignored, IOException e) {
	...
	private
	boolean haveSampleForOneEnabledTrack() {
	...
	private
	void discardSamplesForDisabledTracks(long timeUs) {
		for(int i = 0; i < trackEnabledStates.length; i++) {
	...
	class ExtractingLoadable implements Loadable {
		...
		public
		ExtractingLoadable(Uri uri, DataSource dataSource, Extractor extractor,
