 ...
 */
public
class DashChunkSource implements ChunkSource {

	/**
	 * Thrown when an AdaptationSet is missing from the MPD.
	 */
	public static
	class NoAdaptationSetException extends IOException {

		public
		NoAdaptationSetException(String message) {
			super(message);
		}
	 ...
	 */
	public
	DashChunkSource(DataSource dataSource, FormatEvaluator formatEvaluator,
	 ...
	 */
	public
	DashChunkSource(MediaPresentationDescription manifest, int adaptationSetIndex,
		...
		if(chunk instanceof InitializationChunk) {
			...
			if(initializationChunk.hasSeekMap()) {
				representationHolder.segmentIndex = new DashWrappingSegmentIndex(
						                                                                (ChunkIndex)initializationChunk
								                                                                            .getSeekMap(),
						                                                                initializationChunk.dataSpec.uri
								                                                                .toString());
			}
	...
	private static
	boolean mimeTypeIsWebm(String mimeType) {
		return mimeType.startsWith(MimeTypes.VIDEO_WEBM) || mimeType.startsWith(MimeTypes.AUDIO_WEBM);
	}

	private
	Chunk newInitializationChunk(RangedUri initializationUri, RangedUri indexUri,
	...
	private
	Chunk newMediaChunk(RepresentationHolder representationHolder, DataSource dataSource,
		...
		if(representation.format.mimeType.equals(MimeTypes.TEXT_VTT)) {
			if(representationHolder.vttHeaderOffsetUs != presentationTimeOffsetUs) {
				...
				headerBuilder.append(WebvttParser.EXO_HEADER)
				             .append("=")
				             .append(WebvttParser.OFFSET)
				             .append(presentationTimeOffsetUs)
				             .append("\n");
	 ...
	 */
	private
	long getLiveSeekPosition(long nowUs, boolean indexUnbounded, boolean indexExplicit) {
	...
	private static
	Representation[] getFilteredRepresentations(MediaPresentationDescription manifest,
	...
	private static
	DrmInitData getDrmInitData(MediaPresentationDescription manifest,
	...
	private static
	MediaPresentationDescription buildManifest(List<Representation> representations) {
