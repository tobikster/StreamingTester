 ...
 */
public final
class ContentDataSource implements UriDataSource {

	/**
	 * Thrown when an {@link IOException} is encountered reading from a content URI.
	 */
	public static
	class ContentDataSourceException extends IOException {
	 ...
	 */
	public
	ContentDataSource(Context context) {
		this(context, null);
	}
	 ...
	 */
	public
	ContentDataSource(Context context, TransferListener listener) {
	...
	@Override
	public
	long open(DataSpec dataSpec) throws ContentDataSourceException {
		try {
	...
	@Override
	public
	int read(byte[] buffer, int offset, int readLength) throws ContentDataSourceException {
		if(bytesRemaining == 0) {
			return -1;
		}
		else {
	...
	@Override
	public
	void close() throws ContentDataSourceException {
		uriString = null;
		if(inputStream != null) {
			try {
				inputStream.close();
