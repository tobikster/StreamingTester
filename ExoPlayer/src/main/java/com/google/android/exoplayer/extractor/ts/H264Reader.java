...
/* package */ class H264Reader extends ElementaryStreamReader {
	...
	public
	H264Reader(TrackOutput output, SeiReader seiReader) {
		super(output);
	...
	private
	void feedNalUnitTargetBuffersStart(int nalUnitType) {
		if(!hasOutputFormat) {
	...
	private
	void feedNalUnitTargetBuffersData(byte[] dataArray, int offset, int limit) {
		if(!hasOutputFormat) {
	...
	private
	void feedNalUnitTargetEnd(long pesTimeUs, int discardPadding) {
	...
	private
	void parseMediaFormat(NalUnitTargetBuffer sps, NalUnitTargetBuffer pps) {
	...
	private
	void skipScalingList(ParsableBitArray bitArray, int size) {
		int lastScale = 8;
		int nextScale = 8;
	 ...
	 */
	private
	int unescapeStream(byte[] data, int limit) {
		int position = 0;
	...
	private
	int findNextUnescapeIndex(byte[] bytes, int offset, int limit) {
	...
	class NalUnitTargetBuffer {
		 ...
		 */
		public
		boolean endNalUnit(int discardPadding) {
			if(!isFilling) {
				return false;
			}
