...
/* package */ final
	 ...
	 */
	public
	RollingSampleBuffer(Allocator allocator) {
	 ...
	 */
	private
	void dropUpstreamFrom(long absolutePosition) {
	 ...
	 */
	public
	boolean peekSample(SampleHolder holder) {
	 ...
	 */
	public
	boolean skipToKeyframeBefore(long timeUs) {
	 ...
	 */
	public
	boolean readSample(SampleHolder sampleHolder) {
	 ...
	 */
	private
	void readEncryptionData(SampleHolder sampleHolder, SampleExtrasHolder extrasHolder) {
	 ...
	 */
	private
	void readData(long absolutePosition, ByteBuffer target, int length) {
	...
	private
	void readData(long absolutePosition, byte[] target, int length) {
		int bytesRead = 0;
	 ...
	 */
	private
	void dropDownstreamTo(long absolutePosition) {
		int relativePosition = (int)(absolutePosition - totalBytesDropped);
		int allocationIndex = relativePosition / allocationLength;
	 ...
	 */
	private static
	void ensureCapacity(ParsableByteArray byteArray, int limit) {
	 ...
	 */
	public
	int appendData(ExtractorInput input, int length) throws IOException, InterruptedException {
		ensureSpaceForWrite();
	 ...
	 */
	public
	void appendData(ParsableByteArray buffer, int length) {
	 ...
	 */
	public
	void commitSample(long sampleTimeUs, int flags, long position, int size,
	                  byte[] encryptionKey) {
	...
	class InfoQueue {
		 ...
		 */
		public
		long discardUpstreamSamples(int discardFromIndex) {
		 ...
		 */
		public synchronized
		boolean peekSample(SampleHolder holder, SampleExtrasHolder extrasHolder) {
			if(queueSize == 0) {
				return false;
			}
		 ...
		 */
		public synchronized
		long skipToKeyframeBefore(long timeUs) {
		...
		public synchronized
		void commitSample(long timeUs, int sampleFlags, long offset, int size,
		                  byte[] encryptionKey) {
