...
/* package */ final
	 ...
	 */
	public static
	Track parseTrak(Atom.ContainerAtom trak, Atom.LeafAtom mvhd) {
	 ...
	 */
	public static
	TrackSampleTable parseStbl(Track track, Atom.ContainerAtom stblAtom) {
	 ...
	 */
	private static
	long parseMvhd(ParsableByteArray mvhd) {
	 ...
	 */
	private static
	Pair<Integer, Long> parseTkhd(ParsableByteArray tkhd) {
	 ...
	 */
	private static
	int parseHdlr(ParsableByteArray hdlr) {
	 ...
	 */
	private static
	long parseMdhd(ParsableByteArray mdhd) {
	...
	private static
	StsdDataHolder parseStsd(ParsableByteArray stsd, long durationUs) {
	...
	private static
	void parseAvcFromParent(ParsableByteArray parent, int position, int size,
	...
	private static
	Pair<List<byte[]>, Integer> parseAvcCFromParent(ParsableByteArray parent,
	                                                int position) {
	...
	private static
	TrackEncryptionBox parseSinfFromParent(ParsableByteArray parent, int position,
	                                       int size) {
	...
	private static
	float parsePaspFromParent(ParsableByteArray parent, int position) {
		parent.setPosition(position + Atom.HEADER_SIZE);
	...
	private static
	TrackEncryptionBox parseSchiFromParent(ParsableByteArray parent, int position,
	                                       int size) {
		int childPosition = position + Atom.HEADER_SIZE;
		while(childPosition - position < size) {
			parent.setPosition(childPosition);
	...
	private static
	MediaFormat parseMp4vFromParent(ParsableByteArray parent, int position, int size,
	                                long durationUs) {
		parent.setPosition(position + Atom.HEADER_SIZE);
	...
	private static
	void parseAudioSampleEntry(ParsableByteArray parent, int atomType, int position,
	...
	private static
	byte[] parseEsdsFromParent(ParsableByteArray parent, int position) {
		parent.setPosition(position + Atom.HEADER_SIZE + 4);
	...
	class StsdDataHolder {
		...
		public
		StsdDataHolder(int numberOfEntries) {
