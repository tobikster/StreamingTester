 ...
 */
public final
class Mp4Extractor implements Extractor, SeekMap {
	 ...
	 */
	private
	boolean readAtomPayload(ExtractorInput input, PositionHolder positionHolder)
			throws IOException, InterruptedException {
		parserState = STATE_READING_ATOM_HEADER;
	...
	private
	void processMoovAtom(ContainerAtom moov) {
	 ...
	 */
	private
	int readSample(ExtractorInput input, PositionHolder positionHolder)
			throws IOException, InterruptedException {
	 ...
	 */
	private
	int getTrackIndexOfEarliestCurrentSample() {
	...
	private static
	boolean shouldParseLeafAtom(int atom) {
	...
	class Mp4Track {
		...
		public
		Mp4Track(Track track, TrackSampleTable sampleTable, TrackOutput trackOutput) {
			this.track = track;
