 ...
 */
public final
class TsExtractor implements Extractor, SeekMap {
	...
	public
	TsExtractor(long firstSampleTimestampUs, AudioCapabilities audioCapabilities) {
  ...
  /* package */ long ptsToTimeUs(long pts) {
	 ...
	 */
	private static
	SparseBooleanArray getPassthroughStreamTypes(AudioCapabilities audioCapabilities) {
	...
	class PesReader extends TsPayloadReader {
		...
		public
		PesReader(ElementaryStreamReader pesPayloadReader) {
		 ...
		 */
		private
		boolean continueRead(ParsableByteArray source, byte[] target, int targetLength) {
