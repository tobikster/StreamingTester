 ...
 */
public final
class WebmExtractor implements Extractor {
	...
	private
	void outputSampleMetadata(TrackOutput trackOutput) {
	 ...
	 */
	private
	MediaFormat buildVideoFormat() throws ParserException {
	 ...
	 */
	private
	MediaFormat buildAudioFormat() throws ParserException {
		if(audioTrackFormat != null && CODEC_ID_VORBIS.equals(audioTrackFormat.codecId)) {
			...
			opusInitializationData.add(
					                          ByteBuffer.allocate(Long.SIZE)
					                                    .putLong(audioTrackFormat.codecDelayNs)
					                                    .array());
			opusInitializationData.add(
					                          ByteBuffer.allocate(Long.SIZE)
					                                    .putLong(audioTrackFormat.seekPreRollNs)
					                                    .array());
	 ...
	 */
	private
	ChunkIndex buildCues() throws ParserException {
		if(segmentContentPosition == UNKNOWN) {
	 ...
	 */
	private
	ArrayList<byte[]> parseVorbisCodecPrivate() throws ParserException {
		try {
	 ...
	 */
	private
	boolean maybeSeekForCues(PositionHolder seekPosition, long currentPosition) {
		if(seekForCues) {
	...
	private
	long scaleTimecodeToUs(long unscaledTimecode) {
		return TimeUnit.NANOSECONDS.toMicros(unscaledTimecode * timecodeScale);
	}

	private
	boolean isCodecSupported(String codecId) {
	 ...
	 */
	private final
	class InnerEbmlReaderOutput implements EbmlReaderOutput {
