 ...
 */
public
class HlsSampleSource implements SampleSource, Loader.Callback {

	/**
	 * Interface definition for a callback to be notified of {@link HlsSampleSource} events.
	 */
	public
	interface EventListener extends BaseChunkSampleSourceEventListener {
	}
	...
	public
	HlsSampleSource(HlsChunkSource chunkSource, boolean frameAccurateSeeking,
	...
	@Override
	public
	boolean continueBuffering(long playbackPositionUs) throws IOException {
		Assertions.checkState(prepared);
	...
	private
	boolean continueBufferingInternal() throws IOException {
		maybeStartLoading();
	...
	@Override
	public
	int readData(int track, long playbackPositionUs, MediaFormatHolder formatHolder,
	...
	@Override
	public
	void onLoadError(Loadable loadable, IOException e) {
	 ...
	 */
	private
	HlsExtractorWrapper getCurrentExtractor() {
		HlsExtractorWrapper extractor = extractors.getFirst();
	...
	private
	void discardSamplesForDisabledTracks(HlsExtractorWrapper extractor, long timeUs) {
		if(!extractor.isPrepared()) {
			return;
		}
	...
	private
	boolean haveSamplesForEnabledTracks(HlsExtractorWrapper extractor) {
		if(!extractor.isPrepared()) {
			return false;
		}
		for(int i = 0; i < trackEnabledStates.length; i++) {
	...
	private
	void maybeThrowLoadableException() throws IOException {
	...
	private
	void restartFrom(long positionUs) {
		pendingResetPositionUs = positionUs;
	...
	private
	long getRetryDelayMillis(long errorCount) {
		return Math.min((errorCount - 1) * 1000, 5000);
	}

	protected final
	int usToMs(long timeUs) {
		return (int)(timeUs / 1000);
	}

	private
	void notifyLoadStarted(final long length, final int type, final int trigger,
	...
	private
	void notifyLoadCompleted(final long bytesLoaded, final int type, final int trigger,
	                         final Format format, final long mediaStartTimeUs, final long mediaEndTimeUs,
	...
	private
	void notifyLoadCanceled(final long bytesLoaded) {
		if(eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public
	...
	private
	void notifyLoadError(final IOException e) {
		if(eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public
	...
	private
	void notifyDownstreamFormatChanged(final Format format, final int trigger,
