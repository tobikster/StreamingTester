 ...
 */
public
class WebvttParser implements SubtitleParser {
	...
	public
	WebvttParser(boolean strictParsing, boolean filterTags) {
	...
	@Override
	public
	WebvttSubtitle parse(InputStream inputStream, String inputEncoding, long startTimeUs)
	throws IOException {
		...
		while(true) {
			...
			if(line.startsWith("X-TIMESTAMP-MAP")) {
				...
				if(!timestampMatcher.find()) {
					...
					mediaTimestampUs = (Long.parseLong(timestampMatcher.group()
					                                                   .substring(7)) * 1000)
	...
	protected
	long getAdjustedStartTime(long startTimeUs) {
		return startTimeUs;
	}

	protected
	String processCueText(String line) {
		if(filterTags) {
	...
	protected
	void handleNoncompliantLine(String line) throws ParserException {
		if(strictParsing) {
	...
	private static
	long parseTimestampUs(String s) throws NumberFormatException {
	...
	class WebvttCue {
		...
		public
		WebvttCue(long startTime, long endTime, String text) {
