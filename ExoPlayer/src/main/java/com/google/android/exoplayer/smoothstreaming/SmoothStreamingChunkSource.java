 ...
 */
public
class SmoothStreamingChunkSource implements ChunkSource {
	 ...
	 */
	public
	SmoothStreamingChunkSource(SmoothStreamingManifest manifest, int streamElementIndex,
	...
	private
	StreamElement getElement(SmoothStreamingManifest manifest) {
		return manifest.streamElements[streamElementIndex];
	}

	private
	int getTrackIndex(Format format) {
	...
	private static
	MediaFormat getMediaFormat(StreamElement streamElement, int trackIndex) {
	...
	private static
	MediaChunk newMediaChunk(Format formatInfo, Uri uri, String cacheKey,
	...
	byte[] getKeyId(byte[] initData) {
		...
		String keyIdString = initDataString.substring(
				                                             initDataString.indexOf("<KID>") + 5, initDataString
						                                                                                  .indexOf("</KID>"));
	...
	private static
	void swap(byte[] data, int firstPosition, int secondPosition) {
