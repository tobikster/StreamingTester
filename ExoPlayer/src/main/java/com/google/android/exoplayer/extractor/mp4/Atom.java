...
/* package*/ abstract
	...
	class LeafAtom extends Atom {
		...
		public
		LeafAtom(int type, ParsableByteArray data) {
			super(type);
			this.data = data;
		}
	...
	class ContainerAtom extends Atom {
		...
		public
		ContainerAtom(int type, long endByteOffset) {
			super(type);
		...
		public
		void add(LeafAtom atom) {
			leafChildren.add(atom);
		}

		public
		void add(ContainerAtom atom) {
			containerChildren.add(atom);
		}

		public
		LeafAtom getLeafAtomOfType(int type) {
		...
		public
		ContainerAtom getContainerAtomOfType(int type) {
	 ...
	 */
	public static
	int parseFullAtomVersion(int fullAtomInt) {
	 ...
	 */
	public static
	int parseFullAtomFlags(int fullAtomInt) {
	 ...
	 */
	public static
	String getAtomTypeString(int type) {
		return "" + (char)(type >> 24)
				       + (char)((type >> 16) & 0xFF)
				       + (char)((type >> 8) & 0xFF)
				       + (char)(type & 0xFF);
