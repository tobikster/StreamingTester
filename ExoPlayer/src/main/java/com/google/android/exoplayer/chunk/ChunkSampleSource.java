 ...
 */
public
class ChunkSampleSource implements SampleSource, Loader.Callback {
	...
	public
	ChunkSampleSource(ChunkSource chunkSource, LoadControl loadControl,
	...
	@Override
	public
	boolean continueBuffering(long positionUs) throws IOException {
		Assertions.checkState(state == STATE_ENABLED);
		downstreamPositionUs = positionUs;
	...
	void seekToUs(long positionUs) {
		...
		if(seekInsideBuffer) {
			...
			while(haveSamples && mediaChunks.size() > 1
					      && mediaChunks.get(1)
					                    .getFirstSampleIndex() <= sampleQueue.getReadIndex()) {
				mediaChunks.removeFirst();
			}
		}
		else {
	 ...
	 */
	protected
	void onSampleRead(MediaChunk mediaChunk, SampleHolder sampleHolder) {
		// Do nothing.
	}
	 ...
	 */
	private
	boolean discardUpstreamMediaChunks(int queueLength) {
	...
	private
	boolean isMediaChunk(Chunk chunk) {
	...
	private
	void notifyUpstreamDiscarded(final long mediaStartTimeUs, final long mediaEndTimeUs) {
		if(eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public
