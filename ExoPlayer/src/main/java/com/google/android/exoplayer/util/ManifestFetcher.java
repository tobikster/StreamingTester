 ...
 */
public
class ManifestFetcher<T> implements Loader.Callback {
	 ...
	 */
	public
	ManifestFetcher(String manifestUrl, UriDataSource uriDataSource,
	 ...
	 */
	public
	void updateManifestUrl(String manifestUrl) {
		this.manifestUrl = manifestUrl;
	}
	 ...
	 */
	public
	void singleLoad(Looper callbackLooper, final ManifestCallback<T> callback) {
	...
	private
	void notifyManifestError(final IOException e) {
		if(eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public
	...
	class SingleFetchHelper implements Loader.Callback {
		...
		public
		SingleFetchHelper(UriLoadable<T> singleUseLoadable, Looper callbackLooper,
