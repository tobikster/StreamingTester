...
class CacheDataSource implements DataSource {
	 ...
	 */
	public
	CacheDataSource(Cache cache, DataSource upstream, boolean blockOnCache,
	 ...
	 */
	public
	CacheDataSource(Cache cache, DataSource upstream, DataSource cacheReadDataSource,
	 ...
	 */
	private
	void openNextSource() throws IOException {
		try {
			DataSpec dataSpec;
			CacheSpan span;
			if(ignoreCache) {
	...
	private
	void closeCurrentSource() throws IOException {
	...
	private
	void handleBeforeThrow(IOException exception) {
