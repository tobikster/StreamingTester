...
class AudioTrack {
	 ...
	 */
	public
	int initialize() throws InitializationException {
	 ...
	 */
	public
	int initialize(int sessionId) throws InitializationException {
	 ...
	 */
	public
	void reconfigure(MediaFormat format) {
	 ...
	 */
	public
	void reconfigure(MediaFormat format, int specifiedBufferSize) {
	 ...
	 */
	public
	int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
	...
	public
	boolean hasEnoughDataToBeginPlayback() {
	...
	@TargetApi(21)
	private static
	void setVolumeV21(android.media.AudioTrack audioTrack, float volume) {
		audioTrack.setVolume(volume);
	}

	@SuppressWarnings("deprecation")
	private static
	void setVolumeV3(android.media.AudioTrack audioTrack, float volume) {
	...
	private
	void releaseKeepSessionIdAudioTrack() {
		if(keepSessionIdAudioTrack == null) {
			return;
		}
	 ...
	 */
	private
	void checkAudioTrackInitialized() throws InitializationException {
	...
	private
	long bytesToFrames(long byteCount) {
		if(isAc3) {
			return
	...
	private
	long framesToDurationUs(long frameCount) {
		return (frameCount * C.MICROS_PER_SECOND) / sampleRate;
	}

	private
	long durationUsToFrames(long durationUs) {
		return (durationUs * sampleRate) / C.MICROS_PER_SECOND;
	}

	private
	void resetSyncParams() {
		smoothedPlayheadOffsetUs = 0;
	...
	class AudioTrackUtil {
		 ...
		 */
		public
		void reconfigure(android.media.AudioTrack audioTrack, boolean isPassthrough) {
		 ...
		 */
		public
		long getPlaybackHeadPositionUs() {
		 ...
		 */
		public
		long getTimestampFramePosition() {
			// Should never be called if updateTimestamp() returned false.
			throw new UnsupportedOperationException();
		}
