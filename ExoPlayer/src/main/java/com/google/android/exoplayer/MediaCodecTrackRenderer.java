...
@TargetApi(16)
public abstract
class MediaCodecTrackRenderer extends TrackRenderer {
	 ...
	 */
	public static
	class DecoderInitializationException extends Exception {
		...
		public
		DecoderInitializationException(MediaFormat mediaFormat, Throwable cause, int errorCode) {
		...
		public
		DecoderInitializationException(MediaFormat mediaFormat, Throwable cause,
		                               String decoderName) {
		...
		@TargetApi(21)
		private static
		String getDiagnosticInfoV21(Throwable cause) {
		...
		private static
		String buildCustomDiagnosticInfo(int errorCode) {
	 ...
	 */
	public
	MediaCodecTrackRenderer(SampleSource source, DrmSessionManager drmSessionManager,
	...
	@Override
	protected
	int doPrepare(long positionUs) throws ExoPlaybackException {
		try {
	...
	@Override
	protected
	void onEnabled(long positionUs, boolean joining) {
	 ...
	 */
	protected
	void configureCodec(MediaCodec codec, String codecName,
	...
	protected final
	void maybeInitCodec() throws ExoPlaybackException {
		if(!shouldInitCodec()) {
			return;
		}
	...
	private
	void notifyAndThrowDecoderInitError(DecoderInitializationException e)
	...
	@Override
	protected
	void seekTo(long positionUs) throws ExoPlaybackException {
		currentPositionUs = positionUs;
	...
	@Override
	protected
	void doSomeWork(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
		try {
	...
	private
	void readFormat() throws IOException, ExoPlaybackException {
	...
	private
	void checkForDiscontinuity() throws IOException, ExoPlaybackException {
		if(codec == null) {
			return;
		}
	...
	private
	void flushCodec() throws ExoPlaybackException {
		codecHotswapTimeMs = -1;
		inputIndex = -1;
		outputIndex = -1;
		waitingForFirstSyncFrame = true;
	 ...
	 */
	private
	boolean feedInputBuffer(boolean firstFeed) throws IOException, ExoPlaybackException {
		if(inputStreamEnded
	...
	private static
	MediaCodec.CryptoInfo getFrameworkCryptoInfo(SampleHolder sampleHolder,
	...
	private
	boolean shouldWaitForKeys(boolean sampleEncrypted) throws ExoPlaybackException {
		if(!openedDrmSession) {
			return false;
		}
	 ...
	 */
	protected
	void onInputFormatChanged(MediaFormatHolder formatHolder) throws ExoPlaybackException {
	 ...
	 */
	protected
	void onOutputFormatChanged(MediaFormat inputFormat,
	 ...
	 */
	protected
	boolean canReconfigureCodec(MediaCodec codec, boolean codecIsAdaptive,
	...
	@SuppressWarnings("deprecation")
	private
	boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs)
			throws ExoPlaybackException {
	 ...
	 */
	protected abstract
	boolean processOutputBuffer(long positionUs, long elapsedRealtimeUs,
	                            MediaCodec codec, ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo, int bufferIndex,
	                            boolean shouldSkip) throws ExoPlaybackException;

	private
	void notifyDecoderInitializationError(final DecoderInitializationException e) {
	...
	private
	void notifyCryptoError(final CryptoException e) {
		if(eventHandler != null && eventListener != null) {
			eventHandler.post(new Runnable() {
				@Override
				public
	...
	private
	void notifyDecoderInitialized(final String decoderName,
	...
	private
	int getDecodeOnlyIndex(long presentationTimeUs) {
