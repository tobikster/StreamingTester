 ...
 */
public final
class CacheSpan implements Comparable<CacheSpan> {
	...
	public static
	File getCacheFileName(File cacheDir, String key, long offset,
	                      long lastAccessTimestamp) {
		return new File(cacheDir, key + "." + offset + "." + lastAccessTimestamp + SUFFIX);
	}

	public static
	CacheSpan createLookup(String key, long position) {
		return new CacheSpan(key, position, -1, false, -1, null);
	}

	public static
	CacheSpan createOpenHole(String key, long position) {
		return new CacheSpan(key, position, -1, false, -1, null);
	}

	public static
	CacheSpan createClosedHole(String key, long position, long length) {
	 ...
	 */
	public static
	CacheSpan createCacheEntry(File file) {
	...
	private static
	CacheSpan createCacheEntry(String key, long position, long lastAccessTimestamp,
	                           File file) {
