...
/* package */ final
	...
	public static
	TtmlNode buildTextNode(String text) {
		return new TtmlNode(null, applySpacePolicy(text, true), UNDEFINED_TIME, UNDEFINED_TIME);
	}

	public static
	TtmlNode buildNode(String tag, long startTimeUs, long endTimeUs) {
		return new TtmlNode(tag, null, startTimeUs, endTimeUs);
	}

	private
	TtmlNode(String tag, String text, long startTimeUs, long endTimeUs) {
		this.tag = tag;
		this.text = text;
	...
	private
	void getEventTimes(TreeSet<Long> out, boolean descendsPNode) {
	...
	private
	void getText(long timeUs, StringBuilder builder, boolean descendsPNode) {
	 ...
	 */
	private static
	String applySpacePolicy(String in, boolean treatLineFeedAsSpace) {
