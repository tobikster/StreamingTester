...
class SimpleCache implements Cache {
	...
	@Override
	public synchronized
	CacheSpan startReadWrite(String key, long position)
	...
	CacheSpan startReadWriteNonBlocking(CacheSpan lookupSpan) {
	 ...
	 */
	private
	CacheSpan getSpan(CacheSpan lookupSpan) {
	...
	private
	void notifySpanRemoved(CacheSpan span) {
	...
	private
	void notifySpanAdded(CacheSpan span) {
		ArrayList<Listener> keyListeners = listeners.get(span.key);
		if(keyListeners != null) {
			for(int i = keyListeners.size() - 1; i >= 0; i--) {
	...
	private
	void notifySpanTouched(CacheSpan oldSpan, CacheSpan newSpan) {
