 ...
 */
/* package */ final
              class ExoPlayerImplInternal implements Handler.Callback {
	...
	public
	ExoPlayerImplInternal(Handler eventHandler, boolean playWhenReady,
	...
	public
	void setRendererEnabled(int index, boolean enabled) {
	...
	void sendMessage(ExoPlayerComponent target, int messageType, Object message) {
		customMessagesSent++;
		handler.obtainMessage(MSG_CUSTOM, messageType, 0, Pair.create(target, message))
		       .sendToTarget();
	}
	...
	private
	void prepareInternal(TrackRenderer[] renderers) {
		resetInternal();
	...
	private
	void incrementalPrepareInternal() throws ExoPlaybackException {
	...
	private
	boolean rendererReadyOrEnded(TrackRenderer renderer) {
		if(renderer.isEnded()) {
			return true;
		}
		if(!renderer.isReady()) {
	...
	private
	void setPlayWhenReadyInternal(boolean playWhenReady) throws ExoPlaybackException {
		try {
			rebuffering = false;
			this.playWhenReady = playWhenReady;
			if(!playWhenReady) {
	...
	private
	void startRenderers() throws ExoPlaybackException {
		rebuffering = false;
		mediaClock.start();
	...
	private
	void stopRenderers() throws ExoPlaybackException {
		mediaClock.stop();
		for(int i = 0; i < enabledRenderers.size(); i++) {
	...
	private
	void doSomeWork() throws ExoPlaybackException {
	...
	private
	void scheduleNextOperation(int operationType, long thisOperationStartTimeMs,
	                           long intervalMs) {
	...
	private
	void seekToInternal(long positionMs) throws ExoPlaybackException {
		rebuffering = false;
	...
	private
	void stopAndDisable(TrackRenderer renderer) {
		try {
			ensureStopped(renderer);
	...
	private
	void release(TrackRenderer renderer) {
		try {
			renderer.release();
		}
		catch(ExoPlaybackException e) {
	...
	private
	<T> void sendMessageInternal(int what, Object obj)
	...
	private
	void setRendererEnabledInternal(int index, boolean enabled)
	throws ExoPlaybackException {
	...
	private
	void ensureStopped(TrackRenderer renderer) throws ExoPlaybackException {
